...
class MojeDeloScrapper(BaseScrapper):
    def __init__(
        ...
        super().__init__(
            name, enable_backfill, fetch_limit, backfill_limit, run_interval_minutes
        )
    ...
    def fetch_job_previews_since(
        self, timestamp: int, time_filter: TimeFilter
    ) -> list[JobInfo]:
        valid_jobs = []
        end_reached = False
        current_job_count = 0
        ...
        if not self.jobs_cache:
            while not end_reached:
                ...
                res = self.requests.get(
                    JOB_LIST_API_URL, params=params, headers=HEADERS
                ).json()
                ...
                for item in items:
                    try:
                        title = item["title"]
                        locations = item.get("town", {}).get("translation") or item.get(
                            "jobLocationInput"
                        )
    ...
    def fetch(
        self, limit: int, existing_job_hashes: set[str], last_fetch_timestamp: int
    ) -> list[JobInfo]:
        populated_jobs = []
        try:
            job_previews = self.fetch_job_previews_since(
                last_fetch_timestamp, TimeFilter.NEWER_THAN
            )
    ...
    def backfill(
        ...
        try:
            job_previews = self.fetch_job_previews_since(
                last_backfill_timestamp, TimeFilter.OLDER_THAN
            )
