...
class MojeDeloScrapper(BaseScrapper):
    ...
    def populate_job(self, job_preview: JobInfo):
    ...
    def fetch_job_previews_since(self, timestamp: int, time_filter: TimeFilter) -> list[JobInfo]:
        ...
        while not end_reached:
                ...
                for item in items:
                    title = item["title"]
                    locations = item["town"]["translation"]
                    ...
                    ts = int(dt.timestamp())
                    job_info = JobInfo(title, company, ts, link, BASE_URL, country, locations, "")
    ...
    def fetch(self, limit: int, existing_job_hashes: set[str], last_fetch_timestamp: int) -> list[JobInfo]:
        try:
            ...
            self.error(f"Error when fetching content: {e}")
        finally:
